...
- **产出质量是"网文草稿"级别**：AI 生成的章节可以直接阅读，但存在数字不一致（如前后金额对不上）、反派降智、爽文模板化等问题。**你需要审查和修改输出，不要指望一键出书**

### 中等的
- **长篇小说一致性仍有局限**：写到 20+ 章后，LLM 会开始遗忘早期设定的数字细节（具体金额、等级数值等）。状态机和因果债缓解了这个问题，但没有根治
